...
```text
CourseHub/
├── backend/                       # Python/FastAPI 后端与 Docker 全栈编排
...
│   ├── mcp/                       # Course Lookup、RAG、工具治理
│   ├── skills/                    # 可热加载的 CourseHub 回答规范
│   ├── tools/                     # 课程数据构建与知识库导入
│   ├── tests/
│   ├── docker-compose.yml
│   └── .env
...
除非章节另有说明，下面涉及 Python、`.env`、`docker compose`、`tools/`、`data/` 的命令都从后端目录执行：
