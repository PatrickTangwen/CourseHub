...
class MCPToolManager:
    ...
    async def call(
        ...
        *,
        ...
        """调用工具(对外入口)。执行前后发布协议无关的 typed telemetry。"""
        from core.telemetry import ToolCallFinished, ToolCallStarted, emit_telemetry

        await emit_telemetry(ToolCallStarted(tool_name=name))
        t0 = time.monotonic()
        success = False
        try:
            ...
            await emit_telemetry(ToolCallFinished(
                tool_name=name,
                success=success,
    ...
    async def rewrite_query(self, query: str, n: int = 3) -> List[str]:
        ...
        示例：
          原始: "CSE 100 先修课"
          改写: ["CSE 100 prerequisites", "CSE 100 选课限制", "修 CSE 100 前要上什么课"]
        """
