...
[← 返回中文项目主页](../README.zh-CN.md) · [English homepage](../README.md)

本文档说明 CourseHub 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

Windows 本机的快速启动、重启和健康检查流程见 [`runbooks/本地启动服务指南.md`](runbooks/本地启动服务指南.md)。
...
├── docs/                          # CourseHub 完整指南、specs、ADR 与运维 runbook
...
**前端**:`http://localhost/` 即 ChatGPT 式聊天界面(仓库 `frontend/`,React + assistant-ui,构建进 nginx 镜像);`http://localhost/dev` 是隐藏的开发者面板(知识库/monitor/skills)。前端经 `/api/*` 前缀访问后端,其中 `/api/chat/stream` 为阶段事件 SSE(nginx 已关闭缓冲)。前端开发流程见 [`../frontend/README.md`](../frontend/README.md)。
