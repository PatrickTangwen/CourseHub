...
## 5. Dev 面板(侧边栏底部入口,`/dev` 深链保留)

与聊天同处一个界面:侧边栏底部入口把主区从聊天换成面板(会话列表始终可见,点任一会话或"新建会话"即交回聊天),`/dev` 仍可直达并落在同一布局里。收纳:知识库(add/upload/stats)、monitor 摘要(含工具成功率/延迟)、skills 列表与热重载；每个 Skill 行可点击展开 agents、状态、来源、触发关键词和完整规则正文。无鉴权——**真正的暴露面是它依赖的后端端点,不是这个入口**;生产 nginx 可选择不代理那些端点(见 nginx.conf 注释)。`/eval/run` 不进面板。
