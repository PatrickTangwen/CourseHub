...
- **双语活在对话内容里**:chrome 保持英文(与真实页一致),预录会话中英混排;**不加语言切换钮**。
...
- **初始状态**:首次访问把六条完成态会话播种进 localStorage(走真实 threadStore 机制),落地选中修课规划条(表格 + 免责声明 + 可展开时间线);再次访问不重复播种。
- **自由输入(Demo Notice)**:输入含任一 CJK 字符 → 中文 Notice,否则英文;Notice 是"说明"不是假装的模型回答——无过程时间线,附可点的建议问题;不做关键词模糊匹配。
- **开发者面板**:实录快照渲染;添加文档/上传/重载技能**假成功**,作用于内存状态(统计数字真实变化、通知条显示),刷新复位。
...
- GitHub Actions:push `main` → 构建 demo → 发布 Pages;地址 `https://patricktangwen.github.io/CourseHub/`。
...
1. 打开 Pages 地址,落地即见修课规划会话完整渲染(表格、免责声明、收起的时间线可展开),侧边栏六条会话,中英混排。
2. 点击示例提问,时间线以等比压缩节奏实时逐阶段推进,答案打字机呈现。
3. 中文自由输入得中文 Demo Notice,英文得英文;规范化后精确匹配剧本问题则回放对应会话。
4. `/dev` 呈现实录快照;添加文档假成功且统计数字变化,刷新后复位;健康点恒 Connected。
